...
# Problema: EAS Build intenta usar npm ci pero el proyecto esta configurado para usar yarn
...
# 2. Verificar si yarn esta instalado
try {
...
# 3. Generar yarn.lock con la version correcta de Node
...
# 5. Actualizar eas.json para asegurar configuracion correcta
Write-Host "⚙️ Verificando configuracion de eas.json..." -ForegroundColor Yellow
$easConfig = @{
    cli = @{
        version = ">= 11.0.0"
...
# Asegurar que yarn este disponible
...
Write-Host "  3. ✅ Actualizado eas.json con configuracion correcta"
...
Write-Host ""
Write-Host "📝 Nota: Si aun tienes problemas, asegurate de:" -ForegroundColor Cyan
Write-Host "  - Tener la version correcta de Node (20.11.1)"
Write-Host "  - Usar yarn localmente para todas las instalaciones"
Write-Host "  - Limpiar la cache de EAS si es necesario: eas build:clear-cache"
Write-Host ""
Write-Host "🔧 Comandos adicionales utiles:" -ForegroundColor Magenta
Write-Host "  - Verificar version de Node: node --version"
Write-Host "  - Verificar version de Yarn: yarn --version"
Write-Host "  - Limpiar cache de yarn: yarn cache clean"
