...
export class FirestoreTaskService {
  ...
  private static normalizeTaskData(data: any, id?: string): Task {
    ...
    const scheduledDate = data.scheduledDate
      || data.scheduled_date
      || (typeof data.scheduledWithTime === 'string' ? data.scheduledWithTime.split('T')[0] : undefined)
      || (typeof data.scheduled_with_time === 'string' ? data.scheduled_with_time.split('T')[0] : undefined)
      || (typeof scheduledFor === 'string' ? scheduledFor.split('T')[0] : undefined);

    const scheduledTime = data.scheduledTime
      || data.scheduled_time
      || (typeof data.scheduledWithTime === 'string' ? data.scheduledWithTime.split('T')[1]?.slice(0, 5) : undefined)
      || (typeof data.scheduled_with_time === 'string' ? data.scheduled_with_time.split('T')[1]?.slice(0, 5) : undefined)
