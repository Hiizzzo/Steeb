...
const AuthScreen: React.FC<AuthScreenProps> = ({ onComplete }) => {
  const [mode, setMode] = useState<'welcome' | 'login' | 'register' | 'onboarding'>('welcome');
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Después del login con Google, ir directo al onboarding
      setMode('onboarding');
    } catch (err) {
        ...
        >
          ...
          <div className="space-y-4">
            <button
              ...
              disabled={isLoading}
              className="w-full flex items-center justify-center gap-3 p-3 bg-white dark:bg-gray-800 border-2 border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
            >
      ...
      >
        ...
        <form onSubmit={mode === 'login' ? handleLogin : handleRegister} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full bg-blue-600 text-white py-3 px-4 rounded-xl font-medium hover:bg-blue-700 transition-colors disabled:opacity-50"
          >
