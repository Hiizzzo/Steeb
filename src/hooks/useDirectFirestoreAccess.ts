...
/**
 * Hook que lee DIRECTAMENTE de Firestore sin pasar por el backend
 * Esto asegura que los cambios en Firebase se reflejen inmediatamente
 */
export const useDirectFirestoreAccess = () => {
    ...
    const mapTipoUsuarioToRole = (tipoUsuario: string): 'free' | 'premium' => {
        const premiumTypes = ['Black', 'Premium', 'black', 'premium'];
        return premiumTypes.includes(tipoUsuario) ? 'premium' : 'free';
    };

    // Obtener permisos basados en tipoUsuario
    const getPermissions = (tipoUsuario: string, hasDarkVersion: boolean): string[] => {
        const permissions: string[] = [];

        if (tipoUsuario === 'Black' || tipoUsuario === 'Premium' || hasDarkVersion) {
    ...
    const loadUserData = async (userId: string) => {
        try {
            ...
            if (userDoc.exists()) {
                ...
                const permissions = getPermissions(tipoUsuario, hasDarkVersion);
